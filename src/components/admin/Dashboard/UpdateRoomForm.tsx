...
const UpdateRoomForm =  ({open,setOpen,roomTypes, createRoomTypeHandler, roomInitialValues}:any)=> {
        ...
        useEffect(()=>{
            if (roomInitialValues) {
                setInitialValues( {
                    roomType:roomInitialValues.roomType,
                    description:roomInitialValues?.description,
                    content:roomInitialValues?.content?.length>0 ? roomInitialValues?.content : [""],
                    maxNoOfPeople:roomInitialValues?.maxNoOfPeople,
                    price:roomInitialValues?.price,
                    percentOff:roomInitialValues?.percentOff,
                    image:roomInitialValues?.image,
                    currency:roomInitialValues?.currency,
        ...
        const handleSumbit = (values:FormikValues)=>  {
               ...
               UpdateRoom(roomInitialValues.roomType,values).then(
                res=> {
                        setLoading(false)
                        setOpen(false)
        ...
        return <Modal open={open}>
               <div className="flex justify-center w-full z-[999999]">
                        <div className="bg-white w-[90%] px-5 minMd:px-10 py-5 rounded-[10px] minMd:w-[500px] h-[650px] overflow-y-auto">
                                ...
                                <h1 className="text-center font-bold text-[21px] leading-[25px]">Update room details</h1>
                                <p className="text-start">Step {currenStep} of {MAX_STEP}</p>
                                <Formik initialValues={initialValues} onSubmit={handleSumbit}
                                        validationSchema={formValidation} enableReinitialize={true}
                                        render={({ values, errors }) => (
                                                <Form className="w-full  mt-5">
                                                {
                                                        ...
                                                        meta,
                                                              ...
                                                              >
                                                                {
                                                                        roomTypes?.length >0 ? roomTypes?.map((type:any, index:number)=>(
                                                                                <MenuItem key={type._id} selected={field.value?.toLowerCase()===type.name?.toLowerCase()? true : false} 
                                                                                    className="capitalize"  value={type?.name}
                                                                                >
                                                                                        {type?.name}
                                                                                </MenuItem>
                                                                        )):
